...
# constants.
   ...
   result = 0
   for i in range(size) :
      temp= ord(infile.read(1))
      if endian == LITTLE_ENDIAN :
         result |= (temp << (8*i))
      elif endian == BIG_ENDIAN :
         result = (result << 8) | temp
      else :
         raise Exception('read_unsigned', 'Unknown endian specification')
   return result
...
# to the predefined constants.
   ...
   if endian == LITTLE_ENDIAN :
      ...
      raise Exception('write_unsigned', 'Unknown endian specification')
   outfile.write(value.to_bytes(size, end))
...
# printing to the console.  Probably a stupid way to do it.
   ...
   result = ''
   for temp in value :
      temp = chr(temp) if type(temp) is int else temp
      tempval= ord(temp)
...
# aborts via raising an exception on failure.
   ...
   if actual != value.encode() :
      if severity != ERROR_NONE :
         ...
         print(f'Expected "{escape_string(value)}" at position 0x{infile.tell()-len(actual):X} but saw "{escape_string(actual)}".')
      if severity == ERROR_ABORT :
         print('Aborting!')
         raise Exception('assert_string')
      return False
   else :
      return True
...
# error is printed, rather than all of them.
   ...
   if found_error >= 0 :
      if severity != ERROR_NONE :
         ...
         print(f'Expected "\\x00" at position 0x{found_error:X} but found "{escape_string(temp)}".')
      if severity == ERROR_ABORT :
         print('Aborting!')
         raise Exception('assert_zeroes')
      return False
   else :
      return True
